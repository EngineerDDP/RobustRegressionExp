if __name__ == "__main__":
    ...
    for i in range(40):
        sim = LinearSimulation(b=1.0,normal_scale=0.3,bin_scale=10,oneside=True)
        y = sim.predict(x)
        ...
        #nn = [FCLayer(units=1,act=Linear())]
        #loss = TanhLoss()
        #op = GradientDecentOptimizer(loss=loss, layers=nn, learnrate=0.03)
        #model = Model(nn, op, onehot=False, debug=False)

        #model.fit(x_, y_, epochs=400, batch_size=50, minideltaloss=1e-4)
